      ...
      for (int cnt=0; cnt<runs; cnt++) {
        ...
        foreach (Classifier classifier in clList) {
          if(algo.Equals("") || classifier.getClass().getSimpleName().Equals(algo)) {
              ...
              //Console.Write(classifier.getClass().getSimpleName() + "\t");
              //Console.WriteLine(numCorrect + " out of " + test.numInstances() + " correct (" +(100.0 * numCorrect / test.numInstances()) + "%)");
          }
        }
        classify.Stop();
      }
      Console.WriteLine("read: " + read.ElapsedMilliseconds);
      Console.WriteLine("build: " + build.ElapsedMilliseconds);
      Console.WriteLine("classify: " + classify.ElapsedMilliseconds);
      Console.WriteLine("total: " + (read.ElapsedMilliseconds+build.ElapsedMilliseconds+classify.ElapsedMilliseconds));
