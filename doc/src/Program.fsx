...
try
  ...
  for cnt = 1 to runs do
    ...
    classify.Stop()
  printfn "read: %d" read.ElapsedMilliseconds
  printfn "build: %d" build.ElapsedMilliseconds
  printfn "classify: %d" classify.ElapsedMilliseconds
  printfn "total: %d" (read.ElapsedMilliseconds+build.ElapsedMilliseconds+classify.ElapsedMilliseconds)
with
